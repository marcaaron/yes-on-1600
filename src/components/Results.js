...
class Results extends Component {
	...
	getResult(){
		if(this.props.user==='business'){
			...
			const colorBarGreen = {width: `${parseInt((futureCost/this.props.vars[1])*100,10)+10}%`};
			this.setState({futureCost, colorBarGreen});
		}else{
			...
			console.log('fpl200', fpl200);

			if(fpl200 < totalIncome){
				console.log(prem, numberOfAdults);
			...
			console.log('totalPersonalContribution =', totalPersonalContribution);
		}
	}
		...
		if (this.props.user==='business'){
			return (
				...
				<div className="card">
					...
					<div style={this.state.colorBarGreen} className='color-bar green'><span className='color-bar-text'>${this.state.futureCost}</span></div>
